...
export async function seed() {
  const userData = [
    { id: 'USR-001', name: 'Rina Wulandari', email: 'rina.wulandari@company.co.id', role: 'USER_NON_IT', department: 'Finance & Accounting', avatarUrl: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80' },
    { id: 'IT-001', name: 'Budi Santoso', email: 'budi.santoso@it.company.co.id', role: 'IT_WORKER', department: 'IT Support & Hardware', avatarUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80' },
    { id: 'IT-002', name: 'Andi Pratama', email: 'andi.pratama@it.company.co.id', role: 'IT_WORKER', department: 'Software Engineering', avatarUrl: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&auto=format&fit=crop&q=80' },
    { id: 'IT-003', name: 'Dedi Kurniawan', email: 'dedi.kurniawan@it.company.co.id', role: 'IT_WORKER', department: 'Network & Infrastructure', avatarUrl: 'https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&auto=format&fit=crop&q=80' },
    { id: 'IT-004', name: 'Rizal Siregar', email: 'rizal.siregar@it.company.co.id', role: 'IT_WORKER', department: 'DevOps & Database', avatarUrl: 'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=150&auto=format&fit=crop&q=80' },
    { id: 'ADMIN-001', name: 'Administrator IT', email: 'sysadmin@company.co.id', role: 'SYSTEM_ADMIN', department: 'Enterprise IT Governance', avatarUrl: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&auto=format&fit=crop&q=80' },
  ];
  ...
  await db.insert(users).values(userData).onDuplicateKeyUpdate({ set: userData[0] });
  await db.insert(categories).values(categoryData).onDuplicateKeyUpdate({ set: categoryData[0] });
  await db.insert(subcategories).values(subcategoryData).onDuplicateKeyUpdate({ set: subcategoryData[0] });
  await db.insert(vendors).values(vendorData).onDuplicateKeyUpdate({ set: vendorData[0] });
  await db.insert(technicians).values(technicianData).onDuplicateKeyUpdate({ set: technicianData[0] });
